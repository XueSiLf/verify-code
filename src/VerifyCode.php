...
class VerifyCode
{
    ...
    private function writeCurve(): void
    {
        $px = $py = 0;
        // 曲线前部分
        $A = mt_rand(1, intval($this->conf->imageH / 2)); // 振幅
        $b = mt_rand(-intval($this->conf->imageH / 4), intval($this->conf->imageH / 4)); // Y轴方向偏移量
        $f = mt_rand(-intval($this->conf->imageH / 4), intval($this->conf->imageH / 4)); // X轴方向偏移量
        $T = mt_rand($this->conf->imageH, $this->conf->imageL * 2); // 周期
        $w = (2 * M_PI) / $T;
        $px1 = 0; // 曲线横坐标起始位置
        $px2 = mt_rand(intval($this->conf->imageL / 2), intval($this->conf->imageL * 0.8)); // 曲线横坐标结束位置
